...
const Message: React.FC<MessageProps> = ({ msg }) => {
  ...
  return (
    <div
      className={`flex items-start gap-3 my-4 fade-in ${
        isBot ? '' : 'justify-end'
      }`}
    >
      {isBot && (
      ...
      <div
        className={`max-w-md lg:max-w-2xl p-4 rounded-2xl ${
          isBot
            ? 'bg-white text-gray-800 shadow-sm rounded-tl-none dark:bg-gray-700 dark:text-gray-200'
      ...
      >
        ...
        {msg.type === 'table' && <TableComponent data={msg.rawData} />}
      </div>
    </div>
  )
}
